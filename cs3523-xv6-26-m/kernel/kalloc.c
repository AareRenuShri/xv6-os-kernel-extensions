...
kfree(void *pa)
{
  ...
  if(index < NFRAME) {
    ...
    //         struct proc *owner = frames[index].p;
    //         if(owner != 0){
    //             // Decrement owner's resident_pages safely
    //             acquire(&owner->lock);
...
evict_pages(void)
{
  ...
  while(1) {  // loop until a victim is evicted
    acquire(&frametable_lock);
    f = &frames[clock_hand];
    ...
    if(f->in_use && f->p != 0 && f->p->pagetable != 0){
      ...
      if(pte != 0 && (*pte & PTE_V)){
        ...
        if(frames[((uint64)pa - KERNBASE) >> 12].reference_bit == 1){
          ...
          release(&frametable_lock);  // second chance, continue
        } else {
        ...
        release(&frametable_lock);  // PTE not valid, skip
      }
    } else {
      release(&frametable_lock);    // frame not in use, skip
    }
  }
}