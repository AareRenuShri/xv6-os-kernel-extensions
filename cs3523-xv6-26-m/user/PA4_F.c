...
test_pa4_18(void)
{
    ...
    printf("  HI-priority: disk_reads=%d disk_writes=%d avg_disk_latency=%d.%d\n",
    ...
    printf("  LO-priority: disk_reads=%d disk_writes=%d avg_disk_latency=%d.%d\n",
...
test_pa4_19(void)
{
    ...
    getvmstats(pid, &st1);
    printf("  Sequential FCFS: disk_reads=%d disk_writes=%d avg_disk_latency=%d.%d\n",
    ...
    getvmstats(pid, &st2);
    printf("  Scattered SSTF: disk_reads=%d disk_writes=%d avg_disk_latency=%d.%d\n",
    ...
    if (st2.avg_disk_latency >= st1.avg_disk_latency)
        printf("  PASS: cumulative disk latency did not drop\n");
    else
    ...
    printf("  FCFS avg_disk_latency=%d.%d  SSTF avg_disk_latency=%d.%d\n",
    ...
    if (sstf_lat <= fcfs_lat)
        ...
        printf("  NOTE: SSTF avg_disk_latency higher (cumulative includes prior FCFS ops)\n");
...
test_pa4_21(void)
{
    ...
    printf("  Disk stats: reads=%d writes=%d latency=%d.%d\n",
           vm_after.disk_reads, vm_after.disk_writes,
           vm_after.avg_disk_latency/100, vm_after.avg_disk_latency%100);
    ...
    if (vm_after.disk_writes > vm_before.disk_writes)
        printf("  PASS: disk writes increased (%d)\n",
               vm_after.disk_writes - vm_before.disk_writes);
    else
        printf("  FAIL: no new disk writes\n");

    if (vm_after.disk_reads > vm_before.disk_reads)
        printf("  PASS: disk reads increased (%d)\n",
               vm_after.disk_reads - vm_before.disk_reads);
    else
...
test_pa4_22(void)
{
    ...
    if (st_fcfs.avg_disk_latency >= 700)
        printf("  PASS: FCFS disk latency >= C*100\n");
    else
        printf("  FAIL: FCFS disk latency %d < 700\n", st_fcfs.avg_disk_latency);

    if (st_sstf.avg_disk_latency >= 700)
        printf("  PASS: SSTF disk latency >= C*100\n");
    else
        printf("  FAIL: SSTF disk latency %d < 700\n", st_sstf.avg_disk_latency);
